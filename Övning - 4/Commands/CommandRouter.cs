...
public class CommandRouter
{
    ...
    public void Unpark(string regNumber) => Handle(garageManager.TryUnpark(regNumber)); // has input verification

    public void ParkVehicle(VehicleFilter filter, string? garageName) => Handle(garageManager.TryParkVehicle(filter, garageName));

    public void Find(VehicleFilter filter) => Handle(garageManager.ListVehicles(filter));

    internal void ListSpecificGarage(string? v) => Handle(garageManager.TryListSpecificGarage(v));
    internal void CreateGarage(string? garageType, int? garageSize, string? garageName) => Handle(garageManager.TryAddNewGarage(garageType, garageSize, garageName));

    internal void ListSpecificGarageContents(string[]? strings) => Handle(garageManager.TryListSpecificGarage(string.Join(" ", strings)));
